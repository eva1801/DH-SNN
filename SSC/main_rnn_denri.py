...
#DH-SRNN with 1 layer
class RNN_test_1layer(nn.Module):
    def __init__(self,):
        super(RNN_test_1layer, self).__init__()
        n = 200  #network size

        self.rnn_1 = spike_rnn_test_denri_wotanh_R(700,n,tau_ninitializer = 'uniform',low_n = 0,high_n = 4,vth= 1,dt = 1,branch =4,device=device,bias=is_bias)

        self.dense_2 = readout_integrator_test(n,35,dt = 1,device=device,bias=is_bias)
...
#DH-SRNN with 2 layer    
class RNN_test_2layer(nn.Module):
    def __init__(self,):
        super(RNN_test_2layer, self).__init__()
        n = 200 #network size

        self.rnn_1 = spike_rnn_test_denri_wotanh_R(700,n,tau_ninitializer = 'uniform',low_n = 0,high_n = 4,vth= 1,dt = 1,branch =1,device=device,bias=is_bias)
        self.rnn_2 = spike_rnn_test_denri_wotanh_R(n,n,tau_ninitializer = 'uniform',low_n = 0,high_n = 4,vth= 1,dt = 1,branch =1,device=device,bias=is_bias)
...
def train(epochs,criterion,optimizer,scheduler=None):
    ...
    path = 'model/'  # .pth'
    name = 'rnn_denri_branch4_neuron200_1ms_MG_bs200_nl0h4_final_initzeros_seed0'
    for epoch in range(epochs):
        ...
        if valid_acc>best_acc and train_acc>0.800:
            best_acc = valid_acc
            torch.save(model, path+name+str(best_acc)[:7]+'-srnn-ssc.pth')
...
scheduler = StepLR(optimizer, step_size=25, gamma=.1) # 20
